...
if(!gbox.ui) {
	...
	gbox.ui.createProgressBar = function(json) {
		$("#progressbar").remove();
		var append=$("body").append("<div id=\"progressbar\" style=\"display: none; position:absolute; z-index:1000\"><div class=\"progress-label\"></div></div>");
		...
		if(json.timePicker == true) {
			...
			$rangeStart.datepicker({
				...
				onSelect: function (dateText, inst) {
					...
					if(jex.isNull(rangeEndText)) rangeEndText = "9999-12";
					
					//$(json.keyTarget).val(moment(rangeStartText, "YYYY-MM-DD").format("YYYYMMDD") + "," + moment(rangeEndText, "YYYY-MM-DD").format("YYYYMMDD"));
				
					$(json.keyTarget).val(moment(rangeStartText, "YYYY-MM").format("YYYYMM") + "," + moment(rangeEndText, "YYYY-MM").format("YYYYMM"));
			...
			$rangeEnd.datepicker({
				...
				onSelect: function (dateText, inst) {
					...
					$(json.keyTarget).val(moment(rangeStartText, "YYYY-MM-DD").format("YYYYMMDD") + "," + moment(rangeEndText, "YYYY-MM-DD").format("YYYYMMDD"));
					*/
					if(jex.isNull(rangeStartText)) rangeStartText = "1970-01";
			...
			$rangeStart.val(startDate.format("YYYY-MM"));
			//$rangeEnd.val(endDate.format("YYYY-MM-DD"));
			$rangeEnd.val(endDate.format("YYYY-MM"));
			$rangeStart.mask("9999-99");
			$rangeEnd.mask("9999-99");
			$(json.keyTarget).val(moment($rangeStart.val(), "YYYY-MM").format("YYYYMM") + "," + moment($rangeEnd.val(), "YYYY-MM").format("YYYYMM"));
			
			$rangeStart.datepicker( "option", "maxDate", moment().format("YYYY-MM") );
			$rangeEnd.datepicker( "option", "maxDate", moment().format("YYYY-MM") );
